...
export class DirectHandlerClient {
    ...
    async start(request: any) {
        this.originalConsoleLog("[DirectClient] start called with request:", request);

        if (!this.runnerService) {
            throw new Error("Runner service not connected");
        }

        this.originalConsoleLog("[DirectClient] Forwarding start request to runner service");
        return await this.runnerService.start(request);
    }

    async stop() {
        this.originalConsoleLog("[DirectClient] stop called");

        if (!this.runnerService) {
            throw new Error("Runner service not connected");
        }

        this.originalConsoleLog("[DirectClient] Forwarding stop request to runner service");
