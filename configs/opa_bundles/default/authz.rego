...
# NOTES:
# - When creating an object, this is an operation on the parent object, hence the resource_org_id
#   is the parent's org_id.
...
allow if {
	authn
	input.kind == "org"
	input.action == "create-project"
	input.data.project.org_id == input.resource_org_id
	is_org_member_of_resource
}
...
allow if {
	authn
	input.kind == "prj"
	input.action == "create-connection"
	is_org_member_of_resource
}
...
allow if {
	authn
	input.kind == "prj"
	input.action == "create-deployment"
	is_org_member_of_resource
}
