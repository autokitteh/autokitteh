...
func (c *client) List(ctx context.Context, filter sdkservices.ListEventsFilter) ([]sdktypes.Event, error) {
	resp, err := c.client.List(ctx, connect.NewRequest(
		&eventsv1.ListRequest{IntegrationId: filter.IntegrationID.String(), EventType: filter.EventType, OriginalId: filter.OriginalID, IntegrationToken: filter.IntegrationToken},
	))
	if err != nil {
