...
var tools = []server.ServerTool{
	...
	},
	{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			return toolResultTextf(strings.Join(kittehs.Transform(vs, func(v *sdktypes.CheckViolation) string {
	...
	},
	{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if did, err = sdktypes.ParseDeploymentID(kittehs.ZeroIfNil(didArg)); err != nil {
				return toolResultErrorf("deployment_id: %v", err)
			}
			...
			if eid, err = sdktypes.ParseEventID(kittehs.ZeroIfNil(eidArg)); err != nil {
				return toolResultErrorf("event_id: %v", err)
			}
			...
			if bid, err = sdktypes.ParseBuildID(kittehs.ZeroIfNil(bidArg)); err != nil {
				return toolResultErrorf("build_id: %v", err)
			}
	...
	},
	{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err := common.Client().Sessions().Stop(ctx, sid, kittehs.ZeroIfNil(reasonArg), kittehs.ZeroIfNil(forceArg), time.Second*5); err != nil {
				return toolResultErrorf("%v", err)
			}
	...
	},
	{
		...
		Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			did, err := sdktypes.ParseDeploymentID(kittehs.ZeroIfNil(didArg))
			if err != nil {
				return toolResultErrorf("build_id: %v", err)
			}
...
func resolveExistingProject(ctx context.Context, request mcp.CallToolRequest) (sdktypes.Project, error) {
	...
	if p.ID().IsValid() {
		if p, err = common.Client().Projects().GetByID(ctx, p.ID()); err != nil {
			return sdktypes.InvalidProject, fmt.Errorf("%v", err)
		}
	} else if !p.Name().IsValid() {
		return sdktypes.InvalidProject, fmt.Errorf("no id or name provided")
	} else {
	...
	if !p.IsValid() {
		return sdktypes.InvalidProject, fmt.Errorf("project not found")
	}
