...
func Init(svcs Svcs) {
	...
	svcs.Muxes.Main.Auth.HandleFunc("POST /connections/{id}/test", s.testConnection)
	svcs.Muxes.Main.Auth.HandleFunc("POST /connections/{id}/refresh", s.refreshConnection)
	svcs.Muxes.Main.Auth.HandleFunc("GET /connections/{id}/init/{origin}", s.init)
	svcs.Muxes.Main.Auth.HandleFunc("GET /connections/{id}/postinit", s.postInit)
	svcs.Muxes.Main.Auth.HandleFunc("GET /connections/{id}/success", s.success)
}
...
func (s Svc) init(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("%s/%s?cid=%v&origin=%s", s.svcs.Muxes.MainURL, integ.ConnectionURL(), cid, origin), http.StatusFound)
}
