...
func (v *Vars) Get(ctx context.Context, sid sdktypes.VarScopeID, names ...sdktypes.Symbol) (sdktypes.Vars, error) {
	if len(names) == 0 {
		if err := authz.CheckContext(ctx, sid, "read:get-all-vars"); err != nil {
			return nil, err
		}
	} else {
		for _, name := range names {
			if err := authz.CheckContext(ctx, sid, "read:get-var", authz.WithData("name", name)); err != nil {
				return nil, err
			}
		}
	}
...
func (v *Vars) FindConnectionIDs(ctx context.Context, iid sdktypes.IntegrationID, name sdktypes.Symbol, value string) ([]sdktypes.ConnectionID, error) {
	if err := authz.CheckContext(ctx, sdktypes.InvalidConnectionID, "read:find-var-connections-ids", authz.WithData("integration_id", iid), authz.WithData("name", name)); err != nil {
		return nil, err
	}
