...
func (m *triggers) Create(ctx context.Context, trigger sdktypes.Trigger) (sdktypes.TriggerID, error) {
	...
	if cid := trigger.ConnectionID(); cid.IsValid() {
		cpid, err := m.db.GetProjectIDOf(ctx, cid)
		if err != nil {
			return sdktypes.InvalidTriggerID, fmt.Errorf("get project id of connection: %w", err)
		}

		if cpid != trigger.ProjectID() {
			return sdktypes.InvalidTriggerID, sdkerrors.NewInvalidArgumentError("connection does not belong to specified project")
		}
	}
