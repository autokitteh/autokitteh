...
func (s Svc) initSessions() {
	s.Muxes.Aux.Auth.HandleFunc("/sessions", s.sessions)
	s.Muxes.Aux.Auth.HandleFunc("/sessions/{sid}", s.session)
	s.Muxes.Aux.Auth.HandleFunc("/sessions/{sid}/stop", s.stopSession)
}
