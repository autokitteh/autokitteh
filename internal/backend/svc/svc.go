...
func makeFxOpts(cfg *Config, opts RunOptions) []fx.Option {
	return []fx.Option{
		...
		Component("httpauthmiddleware", authhttpmiddleware.Configs, fx.Provide(authhttpmiddleware.New)),
		Component(
			"temporalclient",
		...
		Component(
			"http",
			httpsvc.Configs,
			fx.Provide(func(lc fx.Lifecycle, z *zap.Logger, cfg *httpsvc.Config, wrap authhttpmiddleware.WrapFunc) (HTTPServerAddr, *http.ServeMux, *muxes.Muxes, error) {
				addr, mux, err := httpsvc.New(
					...
					[]httpsvc.RequestLogExtractor{
						func(r *http.Request) []zap.Field {
							if userID := authcontext.GetAuthnUserID(r.Context()); userID.IsValid() {
		...
		fx.Invoke(func(lc fx.Lifecycle, z *zap.Logger, addr HTTPServerAddr) {
			HookSimpleOnStart(lc, func() {
				sayHello(opts, string(addr))
				z.Info("ready")
			})
		}),
	}
