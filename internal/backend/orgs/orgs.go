...
func (os *orgs) GetByName(ctx context.Context, name sdktypes.Symbol) (sdktypes.Org, error) {
	o, err := os.db.GetOrg(ctx, sdktypes.InvalidOrgID, name)
	if err != nil {
		return sdktypes.InvalidOrg, err
	}

	if err := authz.CheckContext(ctx, o.ID(), "read:get", authz.WithConvertForbiddenToNotFound); err != nil {
		return sdktypes.InvalidOrg, err
	}
...
func (o *orgs) GetOrgsForUser(ctx context.Context, uid sdktypes.UserID) ([]sdktypes.OrgID, error) {
