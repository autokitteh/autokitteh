...
type Event struct {
	...
	Trigger    *Trigger    `gorm:"constraint:OnDelete:Cascade Deferrable"` //TODO: do we want to delete events if we delete triggers ? why do we need this foreign key?
...
type Session struct {
	...
	Project    *Project    `gorm:"constraint:OnDelete:Cascade Deferrable"`
	Event      *Event      `gorm:"references:EventID;constraint:OnDelete:Cascade Deferrable"`
}
