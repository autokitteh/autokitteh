...
func TestGetOwner(t *testing.T) {
	...
	require.NoError(t, db.SaveBuild(ctx, sdktypes.NewBuild().WithOrgID(oid).WithID(bid), []byte("meow")))
	...
	require.NoError(t, db.CreateSession(ctx, sdktypes.NewSession(bid, loc, nil, nil).WithID(sid).WithOrgID(oid)))
	ids := []sdktypes.ID{
		pid,
		bid,
		cid,
