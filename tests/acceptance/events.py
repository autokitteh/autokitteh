...
def on_http_request(event):
    ...
    """
    pretty_print.print_json("HTTP trigger event", event)
...
def on_github_issue_comment(event):
    ...
    """
    pretty_print.print_json("GitHub issue comment event", event)
...
def on_slack_message(event):
    ...
    """
    pretty_print.print_json("Slack message event", event)
